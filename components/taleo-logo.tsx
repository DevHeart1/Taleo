...
export function TaleoLogo({ className, title }: TaleoLogoProps) {
  return (
    <svg
      className={className}
      viewBox="0 0 320 82"
    ...
    >
      <defs>
        <linearGradient id="taleo-logo-cover" x1="0" y1="0" x2="1" y2="1">
          <stop stopColor="#a78bfa" offset="0%" />
          <stop stopColor="#6366f1" offset="100%" />
        </linearGradient>
        <linearGradient id="taleo-logo-pages" x1="0" y1="0" x2="0" y2="1">
          <stop stopColor="#ffffff" offset="0%" />
          <stop stopColor="#fef3c7" offset="100%" />
        </linearGradient>
        <linearGradient id="taleo-logo-star" x1="0" y1="0" x2="1" y2="1">
      ...
      <g aria-hidden="true" fontFamily="var(--font-geist-sans), Arial Rounded MT Bold, Arial, sans-serif" fontSize="50" fontWeight="1000" letterSpacing="-4">
        {"Taleo".split("").map((letter, index) => (
          <text key={`${letter}-${index}`} x={82 + index * 35} y="57" fill={letterColors[index]}>
            {letter}
          </text>
        ))}
      </g>
